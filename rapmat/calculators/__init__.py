...
def cleanup_calculator_files(calculator) -> None:
    """Remove structure-specific output files generated by the calculator.

    Prevents false read-ins (e.g. VASP taking previous CHGCAR/WAVECAR)
    when evaluating multiple distinct structures iteratively in the same folder.
    """
    ...
    if calc_name == "vasp" and hasattr(calculator, "directory"):
        ...
        "WAVECAR", "WAVECAR.h5",
        "CHGCAR", "CHG",
        "vasprun.xml", "OUTCAR", "OSZICAR",
        "EIGENVAL", "DOSCAR", "PROCAR",
        "IBZKPT", "PCDAT", "XDATCAR",
        "CONTCAR",                  # optional but recommended for different structures
        "vasp.out", "vasp*.lock"    # ASE/VASP temp/stdout files
    ]:
